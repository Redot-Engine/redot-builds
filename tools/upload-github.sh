...
if [ ! -d "${basedir}/releases" ] || [ ! -d "${basedir}/tmp" ]; then
  ...
  echo "  Make sure you're running this script from the root of your godot-build-scripts clone, and that Redot has been built with it."
  exit 1
fi
...
if [ ! -d "${version_path}" ]; then
  ...
  echo "  Make sure you're running this script from the root of godot-build-scripts, and that Redot has been built."
  exit 1
fi
